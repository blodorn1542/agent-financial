...
/**
 * A leased token is used until this close to its expiry, then refused. It
 * cannot be refreshed here, so there is no reason to stop using it early - the
 * margin only covers one request in flight.
 */
...
function create(ctx) {
  ...
  async function getValidAccessToken(tenantId) {
    ...
    // A LEASED row is an access token someone else refreshes - the platform
    // gateway's credential vault, which never lets the refresh token out. It
    // is used as given and never refreshed here: when it runs out, the job
    // that leased it is over, and the next job gets a new lease.
    if (row && row.leased) {
      if (row.access_token && row.access_expires_at &&
          row.access_expires_at - LEASE_MARGIN_MS > Date.now()) {
        return { accessToken: row.access_token, realmId: realmOf(row) };
      }
      throw new Error('Tenant "' + tenantId + '" leased QuickBooks token has expired - start a new run');
    }
  ...
  function isConnected(tenantId) {
    const row = credentials.get(tenantId, PROVIDER);
    return !!(row?.refresh_token || (row?.leased && row.access_token));
  }
